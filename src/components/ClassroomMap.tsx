...
import { ZoomIn, ZoomOut, RotateCcw, ArrowUpDown } from 'lucide-react';
...
export const ClassroomMap: React.FC<ClassroomMapProps> = ({
  ...
  const renderPodium = (marginClass: string) => (
    <div
      className={`relative flex flex-col items-center transition-all duration-200 ${marginClass}`}
    >
      <div
        id="podium-desk"
  ...
  const renderTables = () => (
    <div className="w-full flex items-start justify-center gap-10 md:gap-14 lg:gap-16 relative px-4">
      {/* 
        COLUMN 1 (Screen Left):
        - Default (Podium Top): Group 1 (Row 1), Group 2 (Row 2) [Rotation: +14]
        - Flipped (Podium Bottom): Group 8 (Row 2, back), Group 7 (Row 1, front) [Rotation: -14]
      */}
      <div
        className={`flex flex-col items-center pt-2 transition-all duration-200 ${
      ...
      {/* 
        CENTER COLUMNS:
        - Default (Podium Top): 
            Row 1 (front): Group 3, Group 5
            Row 2 (back):  Group 4, Group 6
        - Flipped (Podium Bottom): 
            Row 2 (back, top):    Group 6, Group 4
            Row 1 (front, lower): Group 5, Group 3
      */}
      <div
      ...
      >
        {!isFlipped ? (
          ...
          <>
            {/* Flipped: Back Row on top (Group 6 & Group 4) */}
            <div className="flex gap-6 md:gap-8 lg:gap-10 items-center">
              {group6 && (
                <GroupTable
                  group={group6}
            ...
            {/* Flipped: Front Row closer to bottom podium (Group 5 & Group 3) */}
            <div className="flex gap-6 md:gap-8 lg:gap-10 items-center">
              {group5 && (
                <GroupTable
                  group={group5}
      ...
      {/* 
        COLUMN 3 (Screen Right):
        - Default (Podium Top): Group 7 (Row 1), Group 8 (Row 2) [Rotation: -14]
        - Flipped (Podium Bottom): Group 2 (Row 2, back), Group 1 (Row 1, front) [Rotation: +14]
      */}
      <div
        className={`flex flex-col items-center pt-2 transition-all duration-200 ${
          showNamesAlways ? 'gap-16 md:gap-20' : 'gap-20'
  ...
  return (
    ...
    >
      {/* Top Map Controls */}
      <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-3 border-b border-slate-100 bg-slate-50/70">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-lg border transition-all cursor-pointer ${
              isFlipped
                ? 'bg-[#125977] text-white border-[#093547] shadow-xs'
                : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'
            }`}
            title="Flip seating orientation (Podium at Top or Bottom)"
          >
            <ArrowUpDown className="w-3.5 h-3.5" />
            <span>{isFlipped ? 'Podium: Bottom (Flipped)' : 'Podium: Top (Default)'}</span>
          </button>
      ...
      {/* Main Floor Canvas */}
      <div
        className={`w-full overflow-x-auto overflow-y-visible p-8 md:p-12 flex justify-center items-center select-none bg-white transition-all duration-200 ${
      ...
      >
        ...
        >
          {/* If Normal (Podium Top), render Podium first */}
          {!isFlipped &&
            renderPodium(showNamesAlways ? 'mb-20' : 'mb-24')}

          {/* TABLE SEATING ARRANGEMENT */}
          {renderTables()}

          {/* If Flipped (Podium Bottom), render Podium after tables */}
          {isFlipped &&
            renderPodium(showNamesAlways ? 'mt-20' : 'mt-24')}
        </div>
      </div>
    </div>
  );
